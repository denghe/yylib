...
namespace yy {
    ...
    // 返回首个出现 1 的 bit 的下标
    inline size_t Calc2n(size_t const& n) {
        assert(n);
#ifdef _MSC_VER
        unsigned long r = 0;
...
# endif
    ...
    inline size_t Round2n(size_t const& n) {
    ...
    struct Span {
        ...
        // 引用一段数据
        Span(void const* const& buf, size_t const& len)
        ...
        // 引用一段数据
        YY_INLINE void Reset(void const* const& buf_, size_t const& len_) {
            buf = (uint8_t*)buf_;
            len = len_;
        }
        ...
        // 引用一个 含有 buf + len 成员的对象的数据
        template<typename T, typename = std::enable_if_t<std::is_class_v<T>>>
        YY_INLINE void Reset(T const& d, size_t const& offset_ = 0) {
        ...
        // 判断数据是否一致
        YY_INLINE bool operator==(Span const& o) const {
        ...
        // 下标只读访问
        YY_INLINE uint8_t const& operator[](size_t const& idx) const {
            assert(idx < len);
            return buf[idx];
        }

        // 下标可写访问
        YY_INLINE uint8_t& operator[](size_t const& idx) {
            assert(idx < len);
            return buf[idx];
        }
    };
    ...
    struct Data_r : Span {
        ...
        // 引用一段数据
        Data_r(void const *const &buf, size_t const &len, size_t const &offset = 0) {
            Reset(buf, len, offset);
        }

        // 引用一个 含有 buf + len 成员的对象的数据
        template<typename T, typename = std::enable_if_t<std::is_class_v<T>>>
        Data_r(T const& d, size_t const& offset = 0) {
            Reset(d.buf, d.len, offset);
        }

        // 引用一段数据
        YY_INLINE void Reset(void const *const &buf_, size_t const &len_, size_t const &offset_ = 0) {
        ...
        // 判断数据是否一致
        YY_INLINE bool operator==(Data_r const &o) {
        ...
        [[nodiscard]] YY_INLINE int ReadBuf(void *const &tar, size_t const &siz) {
            assert(tar);
        ...
        [[nodiscard]] YY_INLINE int ReadBufAt(size_t const &idx, void *const &tar, size_t const &siz) const {
            assert(tar);
        ...
        [[nodiscard]] YY_INLINE void* ReadBuf(size_t const& siz) {
        ...
        [[nodiscard]] YY_INLINE void* ReadBufAt(size_t const& idx, size_t const& siz) const {
        ...
        [[nodiscard]] YY_INLINE int ReadFixed(T &v) {
        ...
        template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T> || std::is_enum_v<T>>>
        [[nodiscard]] YY_INLINE int ReadFixedAt(size_t const &idx, T &v) {
        ...
        template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T> || std::is_enum_v<T>>>
        [[nodiscard]] YY_INLINE int ReadFixedBE(T &v) {
            if (offset + sizeof(T) > len) return __LINE__;
            memcpy(&v, buf + offset, sizeof(T));
        ...
        template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T> || std::is_enum_v<T>>>
        [[nodiscard]] YY_INLINE int ReadFixedBEAt(size_t const &idx, T &v) {
            if (idx + sizeof(T) >= len) return __LINE__;
            memcpy(&v, buf + idx, sizeof(T));
#ifdef __LITTLE_ENDIAN__
            v = BSwap(v);
        ...
        template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T> || std::is_enum_v<T>>>
        [[nodiscard]] YY_INLINE int ReadFixedArray(T* const& tar, size_t const& siz) {
            assert(tar);
        ...
        // 读 变长整数. 返回非 0 则读取失败
        template<typename T>
        [[nodiscard]] YY_INLINE int ReadVarInteger(T &v) {
    ...
    struct Data_rw : Data_r {
        ...
        YY_INLINE void Reset(void const* const& buf_ = nullptr, size_t const& len_ = 0, size_t const& offset_ = 0, size_t const& cap_ = 0) {
            this->Data_r::Reset(buf_, len_, offset_);
            cap = cap_;
        }

        // 预分配空间
        explicit Data_rw(size_t const &cap)
                : cap(cap) {
            assert(cap);
        ...
        // 复制( offset = 0 )
        Data_rw(Span const& s)
            : cap(0) {
            WriteBuf(s.buf, s.len);
        }

        // 复制( offset = 0 )
        Data_rw(void const *const &ptr, size_t const &siz)
            : cap(0) {
            WriteBuf(ptr, siz);
        }
        ...
        // 复制( offset = 0 )
        YY_INLINE Data_rw& operator=(Data_rw const& o) {
            return operator=<Data_rw>(o);
        }

        // 复制含有 buf + len 成员的类实例的数据( offset = 0 )
        template<typename T, typename = std::enable_if_t<std::is_class_v<T>>>
        YY_INLINE Data_rw& operator=(T const& o) {
        ...
        // 交换数据
        YY_INLINE Data_rw &operator=(Data_rw &&o) noexcept {
            std::swap(buf, o.buf);
            std::swap(len, o.len);
            std::swap(cap, o.cap);
        ...
        YY_INLINE bool operator==(Data_rw const &o) const {
            return this->Span::operator==(o);
        }
        ...
        YY_NOINLINE void Reserve(size_t const &newCap) {
        ...
        // 修改数据长度( 可能扩容 )。会返回旧长度
        YY_INLINE size_t Resize(size_t const &newLen) {
            if (newLen > cap) {
        ...
        YY_INLINE void Fill(std::initializer_list<T> const &bytes) {
            Clear();
            Reserve(bytes.size());
            for (auto &&b : bytes) {
                buf[len++] = (uint8_t) b;
        ...
        YY_INLINE void RemoveFront(size_t const &siz) {
            assert(siz <= len);
            if (!siz) return;
            len -= siz;
            if (len) {
        ...
        YY_INLINE void WriteBuf(void const *const &ptr, size_t const &siz) {
        ...
        YY_INLINE void WriteBufAt(size_t const &idx, void const *const &ptr, size_t const &siz) {
            if (idx + siz > len) {
                Resize(idx + siz);
            }
        ...
        template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T> || std::is_enum_v<T>>>
        YY_INLINE void WriteFixedAt(size_t const &idx, T v) {
            if (idx + sizeof(T) > len) {
                Resize(sizeof(T) + idx);
            }
#ifdef __BIG_ENDIAN__
        ...
        template<bool needReserve = true, typename T, typename = std::enable_if_t<std::is_arithmetic_v<T> || std::is_enum_v<T>>>
        YY_INLINE void WriteFixedBE(T v) {
            if constexpr (needReserve) {
                if (len + sizeof(T) > cap) {
                    Reserve<false>(len + sizeof(T));
                }
        ...
        template<typename T, typename = std::enable_if_t<std::is_arithmetic_v<T> || std::is_enum_v<T>>>
        YY_INLINE void WriteFixedBEAt(size_t const &idx, T v) {
            if (idx + sizeof(T) > len) {
                Resize(sizeof(T) + idx);
            }
#ifdef __LITTLE_ENDIAN__
        ...
        template<bool needReserve = true, typename T, typename = std::enable_if_t<std::is_arithmetic_v<T> || std::is_enum_v<T>>>
        YY_INLINE void WriteFixedArray(T const* const& ptr, size_t const& siz) {
            assert(ptr);
            if constexpr (needReserve) {
        ...
        YY_INLINE void WriteVarInteger(T const &v) {
            using UT = std::make_unsigned_t<T>;
            UT u(v);
            if constexpr (std::is_signed_v<T>) {
        ...
        template<bool needReserve = true>
        YY_INLINE size_t WriteJump(size_t const &siz) {
            auto bak = len;
            if constexpr (needReserve) {
                if (len + siz > cap) {
                    Reserve<false>(len + siz);
        ...
        // len 清 0, 可彻底释放 buf
        YY_INLINE void Clear(bool const &freeBuf = false) {
            if (freeBuf && cap) {
                //delete[](buf - reserveLen);
                free(buf - reserveLen);
                buf = nullptr;
    ...
    /**********************************************************************************************************************/
    // 为 Data.Read / Write 提供简单序列化功能( 1字节整数, float/double  memcpy,   别的整数 包括长度 变长. std::string/Data 写 长度 + 内容
    // 这里只针对 “基础类型”
